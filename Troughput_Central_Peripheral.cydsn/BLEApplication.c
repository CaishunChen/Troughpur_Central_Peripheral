...
//                printf("+SCAN_STOP\r\n");
//                printf("%d\r\n",CyBle_GetState());
                API_RESULT=CyBle_GapcConnectDevice(&DeviceInfo[DeviceCount-1].DeviceList);
                if(CYBLE_ERROR_OK==API_RESULT)//打开透传通道
                {
                    DeviceCount=0;
...
//            CTS_Write(CTS_OFF);
            ...
            #ifdef Client
                API_RESULT=CyBle_GapcStartScan(CYBLE_SCANNING_FAST);
//                printf("%d\r\n",API_RESULT);
//                printf("Disconnected\r\n");    
//                while((UART_SpiUartGetTxBufferSize() + UART_GET_TX_FIFO_SR_VALID) != 0);//等待串口缓冲区的数据发送完成                
            #endif
...
//        接收从机发送过来的透传数据            
        case CYBLE_EVT_GATTC_HANDLE_VALUE_NTF:
            ...
            if(handleValueNotification->handleValPair.attrHandle == CYBLE_TROUGHPUT_SERVICE_CUSTOM_CHARACTERISTIC_CHAR_HANDLE)//Notify权限的属性句柄
            {
...
/******************************************************************************
* Function Name: ClientThroughput_Handler
***************************************************************************//**
* 
* 主机透传数据
* \param  NULL
...
* 
******************************************************************************/
void ClentThroughput_Handler(void)
{
    uint8_t Uart_Char=0;
//    uint32 passkey=0;
//    uint32 pow;
...
/******************************************************************************
* Function Name: ServerThroughput_Handler
***************************************************************************//**
* 
* 从机透传数据
* \param  NULL
...
* 
******************************************************************************/
void ServerThroughput_Handler(void)
{
    uint8_t Uart_Char=0;
//    uint32 passkey=0;
//    uint32 pow;
